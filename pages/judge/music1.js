// pages/judge/music1.js
import { useState, useEffect } from 'react';
import JudgeVotingPage from '../../components/JudgeVotingPage';

export default function MusicJudge1() {
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const [password, setPassword] = useState('');
  const [error, setError] = useState('');

  const JUDGE_PASSWORD = process.env.NEXT_PUBLIC_MUSIC_JUDGE1_PASSWORD || 'music1pass';
  const STORAGE_KEY = 'judge_music1_auth';
